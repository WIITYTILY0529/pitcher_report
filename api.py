...
async def generate_report(request: ReportRequest):
    try:
        ...
        if not os.path.exists(csv_file):
            # Try to re-fetch? For now, error
            raise HTTPException(status_code=404, detail="Game data CSV not found. Please fetch first.")
        
        df = pd.read_csv(csv_file)
        
        # We need box_stats for the report. 
        # Since we don't store box_stats in CSV, we might need to re-fetch it or pass it.
        # Let's re-fetch box_stats for simplicity, or just call fetch_game_data again (it saves CSV anyway)
        ...
        report_gen.create_report(request.pitcher_name, boxscore_stats=match_stats, selected_pitches=request.selected_pitches)
